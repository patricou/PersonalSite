(() => {
    ...
    const PHONE = window.PHONE = config => {
        ...
        function startcamera() {
            navigator.getUserMedia(mediaconf, stream => {
                ...
                console.log('Start track : ' + stream.id);
                mystream = stream;
        ...
        function stopcamera() {
            ...
            for (let track of mystream.getTracks()) {
                console.log('Stop track : ' + track.id + " " + track.kind);
                track.stop();
            }
        }
        ...
        function add_ice_route(message) {
            ...
            // Add ICE Candidate Routes
            pc.addIceCandidate(
                new IceCandidate(message.packet), debugcb, debugcb
            );
        }
    ...
    function socket(setup) {
        ...
        function publish(data) {
            ...
            let url = ['https://pubsub.pubnub.com/publish', pubkey, subkey, '0', data.channel, '0', encodeURIComponent(JSON.stringify(data.message))].join('/');
